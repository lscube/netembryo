...
static void mpa_info(mpa_data *, MediaProperties * /*TODO*/);
#endif // DEBUG
...
int get_frame2(uint8 *dst, uint32 dst_nbytes, double *timestamp, InputStream istream, MediaProperties *properties, void *private_data) 
{
	...
	if ( (ret = istream_read(mpa->pkt_len - 4, dst + 4, istream)) < (int32_t)mpa->pkt_len-4 )
...
{
	mpa_input in={ 0, src, src_nbytes };
	uint32 header, mpa_header=0;
	//mpa_data *mpa=(mpa_data *)properties; /*shawill*/
	mpa_data *mpa=(mpa_data *)private_data; /*chicco*/
	int ret;
	uint32 dst_offset=0;
	...
	//printf("mpa->pkt_len=%d - *dst_nbytes=%d - to_cpy=%d - 173 ret=%d - src_nbytes=%d\n",mpa->pkt_len,*dst_nbytes,to_cpy,ret,src_nbytes);
	*dst_nbytes = ret+dst_offset+sizeof(mpa_header);
...
static int mpa_read_id3v2(id3v2_hdr *id3hdr, mpa_input *in, mpa_data *mpa)
{
	uint32 tagsize;
	int ret;
	char *tmp; /*chicco: not usefull for fenice*/

	//printf("mpa_read_id3v2\n");
	
	tagsize=dec_synchsafe_int(id3hdr->size);

#if !HAVE_ALLOCA /*chicco: not usefull for fenice*/
	tmp=calloc(1,tagsize);
#else
	tmp=alloca(tagsize);
#endif //!HAVE_ALLOCA
	
	if ( (ret=mpa_read(tagsize, tmp, in)) != (int)tagsize )
...
static int mpa_decode_header(uint32 header, MediaProperties *properties, mpa_data *mpa)
{
	...
	if(RowIndex > 14) 
	{
		/*
		printf("BAAAAAADDD\n");
		printf("BitRate: RowIndex =%d - ColIndex=%d\n",RowIndex,ColIndex);
		fnc_log(FNC_LOG_DEBUG, "[MPA] 313 buff_data: %X %X %X %X\n", buff_data[0], buff_data[1], buff_data[2], buff_data[3]);
		*/
		return -1;
	}
	...
	if(RowIndex > 2) 
	{
		...
		printf("RESERVED\n");
		printf("BitRate: RowIndex =%d - ColIndex=%d\n",RowIndex,ColIndex);
		printf("SampleRate: RowIndex =%d - ColIndex=%d\n",RowIndex,ColIndex);
		fnc_log(FNC_LOG_DEBUG, "[MPA] 313 buff_data: %X %X %X %X\n", buff_data[0], buff_data[1], buff_data[2], buff_data[3]);
		*/
		return -1;
	}
	...
	//printf("frame_size = %d - pkt_len= %d\n",mpa->frame_size,mpa->pkt_len);
