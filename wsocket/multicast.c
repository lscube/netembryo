...
int mcast_join (int sockfd, const struct sockaddr *sa,
                const char *ifname, unsigned int ifindex, union ADDR *addr)
{
    switch (sa->sa_family) {
#ifndef WIN32
    case AF_INET: {
        ...
        memcpy(&(*addr).mreq_in.ipv4_multiaddr,
...
doioctl:
            ...
            memcpy(&(*addr).mreq_in.imr_interface4,
        ...
        return setsockopt(sockfd, IPPROTO_IP,
                          IP_ADD_MEMBERSHIP,
                          &(*addr).mreq_in.NETmreq,
                          sizeof((*addr).mreq_in.NETmreq));
    }
#ifdef    IPV6
    case AF_INET6: {
        ...
        memcpy(&(*addr).mreq_in6.ipv6_multiaddr,
        ...
        if (ifindex > 0)
            ...
            if (ioctl(sockfd, SIOCGIFADDR, &ifreq) < 0)
                return WSOCK_ERRORIOCTL;
            memcpy(&(*addr).mreq_in6.imr_interface6,
                   &((struct sockaddr_in6 *) &ifreq.ifr_addr)->sin6_addr,
                   sizeof(struct in6_addr));
            /*???*/
        } else {
        ...
        return(setsockopt(sockfd, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP,
               &(*addr).mreq_in6.NETmreq6,
               sizeof((*addr).mreq_in6.NETmreq6)));
    }
#endif
#endif
    default:
